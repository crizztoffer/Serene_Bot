...
async def blackjack_ws_handler(request: web.Request):
    ...
    try:
        ...
        # >>> PATCH 5: kickstart MechanicsMain2 timers on connect so countdowns/action bars start immediately
        if bj_cog and hasattr(bj_cog, "_add_room_active"):
            try:
                bj_cog._add_room_active(room_id)
            except Exception as e:
                logger.debug(f"[/blackjack_ws] _add_room_active error (non-fatal): {e}")
