...
# Command factory with static dropdown options
def build_flag_command(flag_reasons: list[str]) -> app_commands.Command:
    ...
    async def flag_command(
        ...
        try:
            ...
            async with conn.cursor() as cursor:
                ...
                if any(f.get("reason") == selected_reason for f in flags):
                    strike_count = sum(1 for s in strikes if s.get("reason") == selected_reason)
                    strikes.append({
            ...
            await interaction.followup.send(f"✅ **{user.display_name}** flagged for: **{selected_reason}**", ephemeral=True)
...
# Entry point for admin_main.py
def start(admin_group: app_commands.Group, bot):
    reasons = getattr(bot, "flag_reasons", [])
    if not reasons:
        logger.warning("No flag reasons loaded; skipping /serene admin flag registration.")
        return

    flag_cmd = build_flag_command(reasons)
    admin_group.add_command(flag_cmd)
