...
class ConfirmButton(Button):
    def __init__(self):
        super().__init__(label="Confirm Flag", style=discord.ButtonStyle.danger, emoji="🚩")

    async def callback(self, interaction: discord.Interaction):
        view: FlagView = self.view

        if not view.selected_reason or not view.selected_users:
            await interaction.response.send_message("Please select both a reason and at least one user.", ephemeral=True)
            return
        ...
        try:
            ...
            async with conn.cursor() as cursor:
                for user in view.selected_users:
                    ...
                    results.append(f"✅ Flagged {user.display_name} for **{reason}**")
